...
void write_bits_mapped(int data, int size){
	...
	int mapped[16] = {0x0};
	int i =0;
	write_bits_mapped_label1:for(i = 0; data > 0; i++){
		mapped[i] = data % 2;
		data = data/2;
	}
	for(i = 0; i < size-1; i++){
...
void write_bits_mapped2(int data, int size){
	...
	int mapped[16] = {0x0};
	int i =0;
	write_bits_mapped2_label0:for(i = 0; data > 0; i++){
		mapped[i] = data % 2;
		data = data/2;
	}
...
void write_headers(int *bits_written){
    /************************************************ WRITE HEADER ****************************************************/
    /*************************************** IMAGE METADATA ********************************/
    const uint8_t ud_data = 0;
    write_bits(ud_data, 8); // User Defined Data (8 bits)
    *bits_written += 8;

    const uint16_t x_size = NCOLS;
    write_bits_mapped2(x_size, 16); // X Size (16 bits)
    *bits_written += 16;

    const uint16_t y_size = NROWS;
    write_bits_mapped2(y_size, 16); // Y Size (16 bits)
    *bits_written += 16;

    const uint16_t z_size = NBANDS;
    write_bits_mapped2(z_size, 16); // Z Size (16 bits)
    *bits_written += 16;

    const uint8_t sample_type = 0; // Image sample values are unsigned integers
    ...
    *bits_written += 1;
    ...
    const uint8_t large_dyn_range_flag = 0;  //‘0’: dynamic range satisfies D ≤ 16.
    write_bits(large_dyn_range_flag, 1);       //‘1’: dynamic range satisfies D > 16.
    *bits_written += 1;

    const uint16_t dyn_range = 16; //DYNAMIC_RANGE; // = D
    write_bits_mapped2(dyn_range, 4); // Dynamic Range (4 bits)
    *bits_written += 4;

    const uint8_t senc_order = 0; // encoded in band-interleaved order
    write_bits(senc_order, 1); // Sample Encoding Order (1 bit)
    *bits_written += 1;

    const uint16_t int_depth = M;
    ...
    *bits_written += 2;
    ...
    const uint8_t ow_size = 4;
    write_bits_mapped2(ow_size, 3); // Output word size (3 bits)
    *bits_written += 3;

    const uint8_t ecod_type = 0; // Sample-adaptive entropy coder
    ...
    *bits_written += 1;
    ...
    const uint8_t fidelity_ctrl = 0;  //lossless
    write_bits(0, 2);
    *bits_written += 2;
    ...
    *bits_written += 1;
    ...
    const uint8_t n_pbands = 3; // = P
    write_bits_mapped2(n_pbands, 4); // Number of prediction bands (4 bits)
    *bits_written += 4;

    const uint8_t p_mode = 1; // Reduced prediction mode
    ...
    *bits_written += 1;
    ...
    const uint8_t winit_method = 0; // Default weight initialization
    write_bits(winit_method, 1); // Weight initialization method (1 bit)
    *bits_written += 1;

    const uint8_t winit_tabflag = 0; // Weight initialization table is not included in predictor metadata
    write_bits(winit_tabflag, 1); // Weight initialization table flag (1 bit)
    *bits_written += 1;

    const uint8_t w_resolution = 0; // When the default weight initialization is used, this field shall have value ‘00000’
    write_bits(w_resolution, 5); // Weight initialization resolution (5 bits)
    *bits_written += 5;


    /*********************************** Entropy Coder Metadata *********************************/
    const uint8_t ulength_limit = UMAX;
    write_bits_mapped2(ulength_limit, 5); // Unitary length limit (5 bits)
    *bits_written += 5;

    const uint8_t rcount_size = 4; //RESCALING_COUNTER_SIZE; // = delta* - 4
    write_bits_mapped2(rcount_size, 3); // Rescaling counter size (3 bits)
    *bits_written += 3;

    const uint8_t inicount_exp = 1; // = delta0
    write_bits(inicount_exp, 3); // Initial count exponent (3 bits)
    *bits_written += 3;

    const uint8_t acc_ini_const = ACCUMULATOR_INIT; // Accumulator Initialization Table is not included in Entropy Coder Metadata
    write_bits_mapped2(acc_ini_const, 4); // Accumulator Initialization Constant (5 bits)
    *bits_written += 4;

    const uint8_t acc_ini_flag = 0; // Accumulator Initialization Table is not included in Entropy Coder Metadata
